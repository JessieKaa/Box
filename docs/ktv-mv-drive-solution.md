...
- `3.1 / 3.2` 保留最初的方案分析，描述项目原始基线
- `3.3` 补充本轮代码后的最新实现状态，用于记录“已经做到哪里”
...
截至当前工作区，KTV 方案已经从“纯设计”进入“首个可运行版本”，并且主干能力已经落到代码中。

### 已落地能力

#### 入口与页面

- 首页 `HomeActivity` 已新增 `KTV` 入口。
...
- 已新增对应 DAO：
  ...
  - `KtvSongDao`
  - `KtvQueueItemDao`
- `AppDataBase` 已升级到 `version = 5`
- `AppDataManager` 已补 `MIGRATION_4_5`
- `RoomDataManger` 已补齐 KTV 相关增删查改、路径归一化、WebDAV 配置同步、队列状态操作

这意味着 KTV 已不再依赖临时内存态，而是具备了持久化歌库和持久化队列基础。

#### 媒体源接入与扫描

- 已抽象 `MediaLibraryProvider`
- 已实现：
  - `LocalMediaLibraryProvider`
  - `WebDavMediaLibraryProvider`
- 已实现 `KtvLibraryIndexManager`
  - 单线程后台扫描
  - 本地目录递归扫描
  - WebDAV 递归列举
  - 扫描完成后全量刷新当前源歌曲
  - 扫描状态写回 `scanStatus / scanError / lastScanAt`
- `DriveActivity` 已支持将当前本地目录或当前 WebDAV 目录加入 KTV 并立即扫描
- `WebdavDialog` 在修改盘源配置后，已尝试同步 KTV 侧 WebDAV 源配置并触发重扫

#### 歌曲检索与点歌

- 已实现 `KtvSongListActivity`
  - 支持按源查看
  - 支持查看“全部歌曲”
  - 支持关键词搜索
  - 搜索带 250ms debounce
  - 搜索在单独线程池执行
- 已实现 `KtvSongFileNameParser`
  - 基于文件名拆分 `title / artist`
  - 已生成 `initials`
- 歌曲列表交互已经具备：
  - `点击` 立即播放
  - `长按` 加入已点队列
...
- 已实现 `KtvQueueManager`
  ...
  - 清空待播
  - 移除待播
- 已实现 `KtvQueueActivity`
  - 查看队列
  - 重新进入当前播放
  - 删除待播
  - 清空待播
- `PlayActivity` 已接入 `KTV_MODE`
  - 通过 `KtvIntent` 启动
  - 从队列恢复当前项
  - 播放完成自动切下一首
  - 播放失败自动跳下一首
...
如果按本文原来的阶段定义判断，当前状态大致是：

- `P0`：基本完成
- `P1`：已经进入实现，但还没有收口

更准确地说，这一版已经不只是“可搜索并立即播放”，而是已经具备以下闭环：
...
- 结果：失败
- 失败点：`pyramid` 模块缺少可用的 `buildPython` 配置，报错指向 `D:/Programs/Python/Python38/python.exe`

结论：

- 这次失败不指向 KTV 逻辑本身
- 但说明当前仓库还没有完成全量可编译验证
- 后续需要先修复构建环境，再做一次完整回归

### 仍然明显未闭合的问题

下面这些不是增强项，而是当前版本距离“稳定可交付”还差的部分。

#### 1. 搜索能力仍然偏基础

- 当前数据库字段里没有真正的 `pinyin`
- 搜索主要依赖：
  - `title`
  - `artist`
  - `fileName`
  - `searchText`
  - `initials`
- `initials` 来自简单分词首字母，不等于中文拼音索引

结论：当前搜索可用，但还没有达到方案里定义的“拼音点歌”。

#### 2. 队列能力只覆盖基础顺播

- 已有：加入队尾、立即播放、删除待播、清空待播、自动下一首
- 未见：
  - 插播到下一首
  - 队列重排
  - 上一首恢复
...
结论：现在更像“简化排队播放器”，还不是完整 KTV 队列控制台。

#### 3. 扫描策略仍是全量刷新

- `KtvLibraryIndexManager` 当前每次扫描都会：
  - 递归全量枚举
  - 清空该源旧歌曲
  - 重插全量结果
- 还没有：
...
结论：中小歌库可以先跑通，大歌库和慢 WebDAV 仍有性能风险。

#### 4. 媒体源管理还比较粗糙

- `KtvHomeActivity` 目前支持添加与重扫
- 但未见完整的：
  - 删除歌库源 UI
  - 禁用/启用源 UI
  - 修改源名称 UI
...
结论：当前源管理能用，但还不够像正式产品页面。

#### 5. 播放页的 KTV 专用体验还较轻

- `PlayActivity` 已能按 KTV 队列运行
- 但当前主要是逻辑接管，未见明显完成：
  - KTV 专用队列侧栏
  - 更简洁的 KTV 控件集
  - 遥控器点歌场景下的专门提示信息布局

结论：链路打通了，但 UI 还保留明显的 VOD 底色。

#### 6. 稳定性与兼容性还未经过完整验证

- 本地目录访问仍受 Android 存储权限与 ROM 差异影响
- WebDAV 仍依赖目录列举成功、路径兼容、网络稳定
- 当前代码里虽然考虑了失败跳过，但没有看到完整的异常统计与恢复策略

结论：逻辑层面已有兜底，工程层面的设备覆盖还不够。
...
- 不是“仅有方案”
- 也不是“已经收尾”
- 而是“主链路已经做出来，接下来应该进入收口和增强阶段”

如果现在要继续推进，重点不该再放在“大改架构”，而应放在：
